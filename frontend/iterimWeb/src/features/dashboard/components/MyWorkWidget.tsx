...
export function MyWorkWidget({ workItems }: { workItems: DashboardWorkItem[] }) {
  const { t } = useLanguage();
  return (
    <Card className="h-full border-none shadow-none bg-transparent lg:bg-card lg:border lg:shadow-sm flex flex-col">
      ...
      <CardContent className="px-0 lg:px-6 flex-1 min-h-0 overflow-y-auto pr-2">
        {workItems.length === 0 ? (
