...
namespace iterimApi.Services.Implementations 
{
    public class BoardService : IBoardService
    {
        ...
        public async Task<BoardDto?> GetBoardByIterationIdAsync(int teamId, int iterationId)
        {
            // 1. Randame konkrečią iteraciją pagal ID, priskirtą šiai komandai
            var iteration = await _context.Iterations
                ...
                .Where(i => i.TeamId == teamId && i.Id == iterationId) // <--- Ieškome pagal ID, o ne Active statusą
                .FirstOrDefaultAsync();
            ...
            var iterationDto = new IterationDto
            {
                ...
                UpdatedBy = iteration.UpdatedBy,
                CreatedByName = "", 
                UpdatedByName = "",
                WorkItemCount = iteration.WorkItems?.Count ?? 0,
                TotalPoints = iteration.WorkItems?.Sum(wi => wi.Points ?? 0) ?? 0
            };
            ...
            {
                var itemsInStatus = iteration.WorkItems?
